...
func defineSPs(server *samlidp.Server) error {
	for _, data := range idsps {
		...
		err = server.Store.Put(fmt.Sprintf("/services/%s",
			url.PathEscape(metadata.EntityID)),
			&samlidp.Service{
				Metadata: *metadata,
			})
		if err != nil {
